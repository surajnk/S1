...
class StockMove(models.Model):
    ...
    def _action_assign(self):
        ...
        # This is a cosmetic destination-location fixup for MRP components
        # (see commit "Fix PC pick component destination not following first
        # workorder"). _action_assign() can run synchronously inside
        # stock.move._action_confirm() (e.g. for consumable components,
        # which bypass reservation) — i.e. inside the same transaction that
        # creates the MO's Pick Components transfer. An unexpected error
        # here must never roll back that transfer creation, so any failure
        # is logged and swallowed instead of propagated.
        try:
            ...
            _logger.exception(
                "_action_assign: failed to sync component destination to first "
                "workorder's location; leaving reservation/transfer untouched."
            )
