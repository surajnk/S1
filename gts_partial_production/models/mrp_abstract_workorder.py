...
class MrpAbstractWorkorder(models.AbstractModel):
    ...
    def _prepare_component_quantity(self, move, qty_producing):
        ...
        if move.bom_line_id and move.bom_line_id.bom_id.product_qty:
            factor = move.bom_line_id.product_qty / move.bom_line_id.bom_id.product_qty
        else:
            factor = move.unit_factor  # fallback for moves with no linked BOM line

        return move.product_uom._compute_quantity(
            qty_producing * factor,
            uom,
            round=False
        )
    ...
    @api.onchange('qty_producing', 'product_uom_id')
    def _onchange_qty_producing(self):
        ...
        """
        # if self.product_uom_id and self.qty_producing <= 0:
        ...
        for values in line_values['to_create']:
            self.env[self._workorder_line_ids()._name].create(values)
    ...
    @api.onchange('qty_producing', 'product_uom_id')
    def _onchange_qty_producing_check_overissue(self):
        return
    # @api.onchange('qty_producing', 'product_uom_id')
    # def _onchange_qty_producing_check_overissue(self):
    ...
    #         if float_is_zero(issued_qty, precision_rounding=move.product_uom.rounding):
    #             continue
    #         required_qty = self._prepare_component_quantity(move, self.qty_producing)
    #         if float_compare(required_qty, issued_qty, precision_rounding=move.product_uom.rounding) > 0:
    #             lots = ', '.join(
    #                 move.move_line_ids.filtered(lambda ml: ml.lot_id).mapped('lot_id.name')
    #             ) or _('N/A')
    #             return {
    #                 'warning': {
    #                     'title': _('Over-Issue Warning'),
    #                     'message': _(
    #                         'Producing %.2f requires %.2f of %s,\n'
    #                         'but only %.2f was issued (Lot: %s).\n\n'
    #                         'The system will pre-fill the lot(s) from the issued transfer.\n'
    #                         'Please verify the component lines before saving.'
    #                     ) % (
    #                         self.qty_producing,
    #                         required_qty,
    ...
    def _update_workorder_lines(self):
        _logger.info("=== _update_workorder_lines START qty_producing=%s ===", self.qty_producing)
        ...
        """
        line_values = {'to_create': [], 'to_delete': self.env[self._workorder_line_ids()._name], 'to_update': {}}
        # Clean up workorder lines whose move was cancelled (e.g. via unlink()
        # cancel-fallback for confirmed-but-unreserved moves in stock_move.py).
        # Such moves are excluded from the loop below, so their existing lines
        # would otherwise never be flagged for deletion and keep showing in
        # the Produce wizard.
        cancelled_move_lines = self._workorder_line_ids().filtered(
            lambda w: w.move_id and w.move_id.state == 'cancel'
        )
        line_values['to_delete'] |= cancelled_move_lines
        ...
        _logger.info("=== move_raw_ids count=%s states=%s ===", 
            len(move_raw_ids), 
            move_raw_ids.mapped('state')
        )
        for m in self.move_raw_ids._origin:
            _logger.info("=== RAW MOVE id=%s product=%s state=%s qty=%s qty_done=%s ===",
                m.id, m.product_id.name, m.state, m.product_uom_qty, m.quantity_done
            )
        for move in move_raw_ids | move_finished_ids:
            ...
            _logger.info("=== MOVE=%s product=%s unit_factor=%s new_qty_before=%s existing_lines=%s ===",
                move.id, move.product_id.name, move.unit_factor, new_qty, len(move_workorder_lines))
            ...
            _logger.info("=== MOVE=%s new_qty_after=%s qty_todo=%s ===",
                move.id, new_qty, qty_todo)
    ...
    def _generate_lines_values(self, move, qty_to_consume):
        _logger.info("=== _generate_lines_values move=%s product=%s qty=%s move_line_ids=%s ===",
        move.id, move.product_id.name, qty_to_consume, move.move_line_ids.ids)
        lines = []
        ...
        for move_line in move.move_line_ids:
            _logger.info("=== MOVE LINE id=%s lot=%s product_uom_qty=%s qty_done=%s produce_line_ids=%s ===",
                move_line.id, move_line.lot_id.name, move_line.product_uom_qty, 
                move_line.qty_done, move_line.produce_line_ids.ids)
        ...
        for move_line in move.move_line_ids:
            ...
            if move_line.produce_line_ids or float_compare(move_line.product_uom_qty, move_line.qty_done, precision_rounding=move.product_uom.rounding) <= 0:
                continue
        ...
        if float_compare(qty_to_consume, 0.0, precision_rounding=move.product_uom.rounding) > 0:
            if move.product_id.tracking == 'serial':
                ...
                validated_lines = move.move_line_ids.filtered(
                    lambda ml: ml.product_uom_qty > 0 and ml.lot_id
                )
                if validated_lines:
                    for vml in validated_lines:
                        line = dict(initial_line_values)
                        line.update({
                            'move_id': move.id,
                            'product_id': move.product_id.id,
                    ...
                    lines.append(line)
            _logger.info("=== _generate_lines_values RESULT lines_count=%s ===", len(lines))
        return lines
    # @api.model
    # def _generate_lines_values(self, move, qty_to_consume):
    ...
    def _update_moves(self):
        ...
        for abstract_wo in self:
            ...
            for line in workorder_lines_to_process:
                ...
                _logger.info("INSIDDDEE UPDATE MOVESSS \n")                
                if float_compare(line.qty_done, 0, precision_rounding=line.product_uom_id.rounding) > 0:
                    ...
                    _logger.info("INSIDDDEE VALSSS LIST \n")                                   
        ...
        _logger.info("INSIDDDEE STOCKK MOVE LINE CREATE \n")        
...
class MrpAbstractWorkorderLine(models.AbstractModel):
    ...
    def _compute_company_id(self):
        for line in self:
            line.company_id = line._get_production().company_id or line.move_id.company_id
    ...
    def _update_move_lines(self):
        ...
        if self.lot_id:
            ...
            and not ml.produce_line_ids
            and float_compare(
                ml.product_uom_qty, ml.qty_done,
                precision_rounding=ml.product_uom_id.rounding) > 0
            )
            _logger.info("=== GTS _update_move_lines [LOT branch] lot=%s qty_done=%s move_lines=%s ===",
                self.lot_id.name, self.qty_done, move_lines.ids)
        else:
            move_lines = self.move_id.move_line_ids.filtered(
            ...
            _logger.info("=== GTS _update_move_lines [NO-LOT branch] qty_done=%s move_lines=%s ===",
                self.qty_done, move_lines.ids)
        ...
        for ml in move_lines:
            ...
            if float_compare(ml.product_uom_id._compute_quantity(new_quantity_done, ml.product_id.uom_id), ml.product_qty, precision_rounding=rounding) >= 0:
                ml.write({
                    ...
                    # 'produce_line_ids': self._get_produced_lots(),
                })
            else:
