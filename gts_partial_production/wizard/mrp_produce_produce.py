...
class MrpProductProduce(models.TransientModel):
    ...
    def do_produce(self):
        ...
        self.production_id.qty_producing = max(remaining, 0.0)
        #self.production_id.qty_producing = max(remaining, 0.0)
    ...
    def _get_todo(self, production):
        ...
        main_product_moves = production.move_finished_ids.filtered(
            lambda x: x.product_id.id == production.product_id.id
            and not x.picking_id
        )
        to_produce = sum(production.workorder_ids.filtered(
            lambda r: r.show_partial_produce).final_roll_line_ids.mapped("yards_qty"))
        if not to_produce:
            return production.qty_producing or 0.0
        todo_quantity = to_produce - sum(main_product_moves.mapped('quantity_done'))
        return max(todo_quantity, 0.0)
    ...
    def _get_customer_qty(self, production):
        ...
        customer_qty = sum(
            production.workorder_ids.filtered(
                lambda r: r.show_partial_produce).final_roll_line_ids.mapped("total_qty"))
    ...
    def _record_production(self):
        ...
        # Capture existing finished move lines BEFORE _update_finished_move runs
        # so we can identify and remove only the generic line it creates
        # pre_update_ml_ids = set(
        ...
        if not has_yield:
            ...
            self.re_generate_putup_move_line(pre_update_ml_ids=pre_update_ml_ids)
        else:
        ...
        # for mv_line in self.production_id.move_finished_ids.filtered(
        #     lambda m: m.product_id == self.product_id and not m.picking_id
        # ):
        #     if mv_line.state in ('partially_available', 'confirmed', 'assigned'):
        #         mv_line._action_done()

        for mv_line in self.production_id.move_finished_ids.filtered(
            lambda m: m.product_id == self.product_id and not m.picking_id):
            if mv_line.state in ('partially_available', 'confirmed', 'assigned'):
                if not pre_update_ml_ids:
                    mv_line._action_done()
                else:
                    # TEMP DIAGNOSTIC — do not leave this in permanently
                    before_pickings = self.env['stock.picking'].search([
                        ('origin', '=', self.production_id.name)
                    ]).ids
                    _logger.info("=== DIAG before _action_done: pickings=%s mv_line.id=%s mv_line.state=%s ===",
                        before_pickings, mv_line.id, mv_line.state)

                    mv_line._action_done()

                    after_pickings = self.env['stock.picking'].search([
                        ('origin', '=', self.production_id.name)
                    ])
                    _logger.info("=== DIAG after _action_done: pickings=%s (states=%s) new_picking_ids=%s mv_line.state=%s ===",
                        after_pickings.ids,
                        after_pickings.mapped('state'),
                        list(set(after_pickings.ids) - set(before_pickings)),
                        mv_line.state,
                    )

    #def re_generate_putup_move_line(self):
    def re_generate_putup_move_line(self, pre_update_ml_ids=None):
        self.ensure_one()
        ...
        if not finished_moves:
            template_move = mo.move_finished_ids.filtered(
                lambda m: m.product_id == self.product_id
                and not m.picking_id
            ).sorted('id', reverse=True)[:1]
            if not template_move:
                return
            finished_moves = self.env['stock.move'].create({
                'name': template_move.name,
                'product_id': template_move.product_id.id,
                'product_uom': template_move.product_uom.id,
                'product_uom_qty': self.qty_producing,
                'location_id': template_move.location_id.id,
                'location_dest_id': template_move.location_dest_id.id,
                'production_id': mo.id,
                'company_id': template_move.company_id.id,
                'state': 'confirmed',
                'origin': template_move.origin,
                'picking_type_id': template_move.picking_type_id.id,
            })
        # finished_moves = mo.move_finished_ids.filtered(
        #     lambda m: m.product_id == self.product_id and m.state not in ('done', 'cancel'))[:1]
        # if not finished_moves:
        #     return
        ...
        if mo.x_dynamic_putup:
            _logger.info("=== SFP SYNC: MO=%s taking DYNAMIC-PUTUP branch ===", mo.name)
            ...
            #     finished_moves.mapped('move_line_ids').filtered(
            #         lambda ml: ml.state == 'done'
            #         and (not ml.picking_id or ml.picking_id.picking_type_code != 'internal')
            #     ).mapped('qty_done')
            # )
            all_production_moves = mo.move_finished_ids.filtered(
                lambda m: m.product_id == self.product_id
                and not m.picking_id
            )
            already_done_qty = sum(
                all_production_moves.mapped('move_line_ids').filtered(
                    lambda ml: ml.qty_done > 0
                    and ml.id in pre_update_ml_ids
                ).mapped('qty_done')
            )

            _logger.info("=== already_done_qty=%s pre_update_ml_ids=%s all_move_lines=%s ===",
                already_done_qty,
                pre_update_ml_ids,
                [(ml.id, ml.qty_done) for ml in all_production_moves.mapped('move_line_ids')]
            )
            cumulative = 0.0
            ...
            # Get template from ANY finished production move including done ones
            # (needed for session 2+ where the new move has no lines yet)
            template_source = all_production_moves.mapped('move_line_ids')[:1]
            if not template_source:
                raise UserError(_('No existing move line found to use as a template.'))
            template_vals = {
                'company_id': template_source.company_id.id,
                'product_id': template_source.product_id.id,
                'product_uom_id': template_source.product_uom_id.id,
                'location_id': template_source.location_id.id,
                'location_dest_id': template_source.location_dest_id.id,
            }
            lot_required = (template_source.product_id.tracking == 'lot')

            for move in finished_moves:
                _logger.info("=== re_generate existing move lines BEFORE: %s ===",
                    [(ml.id, ml.lot_id.name, ml.qty_done) for ml in move.move_line_ids]
                )

                if pre_update_ml_ids is not None:
                    lines_to_remove = move.move_line_ids.filtered(
                        lambda ml: ml.id not in pre_update_ml_ids
                    )
                    if lines_to_remove:
                        _logger.info("=== removing _update_finished_move lines: %s ===",
                            lines_to_remove.ids)
                        lines_to_remove.unlink()

                existing_ml_ids = move.move_line_ids.ids
                ...
                for roll_line in new_lines:
                    ...
                    if lot_required:
                        lot = self.env['stock.production.lot'].create({
                            'product_id': template_vals['product_id'],
                            'company_id': template_vals['company_id'],
                    ...
                    new_cmds.append((0, 0, {
                        ...
                        'company_id': template_vals['company_id'],
                        'product_id': template_vals['product_id'],
                        'product_uom_id': template_vals['product_uom_id'],
                        'location_id': template_vals['location_id'],
                        'location_dest_id': template_vals['location_dest_id'],
                        'qty_done': qty,
                        'product_uom_qty': qty,
                        'lot_id': lot_id,
                    }))
            ...
            #         }))
                ...
                if new_cmds:
                    move.write({'move_line_ids': new_cmds})

                created_lines |= move.move_line_ids.filtered(
                    lambda ml: ml.id not in existing_ml_ids
                )
            ...
            # Sync to existing SFP transfer — works for both sale and non-sale MOs
            # sfp_picking = self.env['stock.picking'].search([
            #     ('origin', '=', mo.name),
            ...
            # pick_move = sfp_picking.move_ids_without_package.filtered(
            #     lambda m: m.product_id == self.product_id
            # )[:1]
            # if not pick_move:
            #     return
            ...
            if not sfp_picking:
                ...
                _logger.info("=== SFP SYNC (dynamic-putup) fallback: mo.procurement_group_id=%s sale_orders=%s ===",
                    mo.procurement_group_id.id, sale_orders.ids)
                if sale_orders:
                    sale_order = sale_orders[0]
                    candidate_pickings = sale_order.picking_ids.filtered(
                        lambda p: p.location_id.id == mo.location_dest_id.id
                        and p.state not in ('done', 'cancel')
                    )
                    _logger.info("=== SFP SYNC (dynamic-putup) fallback: sale_order=%s candidate_pickings=%s ===",
                        sale_order.name, candidate_pickings.ids)
                    if candidate_pickings:
                        sfp_picking = candidate_pickings[0]

            if not sfp_picking:
                _logger.info("=== SFP SYNC ABORT (dynamic-putup): no sfp_picking for MO=%s ===", mo.name)
                return
            ...
            if not pick_move:
                _logger.info("=== SFP SYNC ABORT (dynamic-putup): no pick_move on picking=%s ===", sfp_picking.name)
                return

            new_total = sum(created_lines.mapped('qty_done')) or 0.0
            if new_total <= 0:
                _logger.info("=== SFP SYNC ABORT (dynamic-putup): new_total<=0, created_lines=%s ===", created_lines.ids)
                return

            _logger.info("=== SFP SYNC PROCEEDING (dynamic-putup): picking=%s pick_move=%s created_lines=%s ===", sfp_picking.name, pick_move.id, created_lines.ids)

            # all_done_qty = sum(all_production_moves.mapped('move_line_ids').mapped('qty_done'))
            # pick_move.write({'product_uom_qty': all_done_qty})

            # all_done_qty = sum(mo.move_finished_ids.filtered(
            #     lambda m: m.product_id == self.product_id and not m.picking_id
            ...
            target_demand = max(mo.product_qty, all_done_qty)
            pick_move.write({'product_uom_qty': target_demand})
            
            pick_cmds = []
            for ml in created_lines:
                pick_cmds.append((0, 0, {
                    'picking_id': sfp_picking.id,
                    'move_id': pick_move.id,
                    'company_id': sfp_picking.company_id.id,
                    'product_id': ml.product_id.id,
                    'product_uom_id': ml.product_uom_id.id,
                    'location_id': sfp_picking.location_id.id,
                    'location_dest_id': sfp_picking.location_dest_id.id,
                    'qty_done': ml.qty_done,
                    # 'product_uom_qty': ml.qty_done,
                    'lot_id': ml.lot_id.id or False,
                    'customer_qty': ml.lot_id.customer_qty or 0.0,
                }))
            if pick_cmds:
                sfp_picking.write({'move_line_ids_without_package': pick_cmds})
            return
            # sale_orders = mo.procurement_group_id.mrp_production_ids.move_dest_ids.group_id.sale_id
            # if not sale_orders:
            #     return
            ...
            # if not picking:
            #     return
            # # Use the picking that already has lines (from session 1) if multiple exist
            # picking = picking.sorted(lambda p: len(p.move_line_ids_without_package), reverse=True)[0]
            ...
            # # all_done_qty = sum(finished_moves.mapped('move_line_ids').mapped('qty_done'))
            # all_done_qty = sum(all_production_moves.mapped('move_line_ids').mapped('qty_done'))
            # pick_move.write({'product_uom_qty': all_done_qty})
        ...
        if mo.x_mrp_order_slit:
            _logger.info("=== SFP SYNC: MO=%s taking MULTI SLIT branch ===", mo.name)
            ...
            sale_orders = mo.procurement_group_id.mrp_production_ids.move_dest_ids.group_id.sale_id
            if not sale_orders:
                _logger.info("=== SFP SYNC ABORT: no sale_orders found for MO=%s ===", mo.name)
                return
            sale_order = sale_orders[0]

            picking = sale_order.picking_ids.filtered(lambda p: p.location_id.id == mo.location_dest_id.id)
            if not picking or len(picking) != 1:
                _logger.info("=== SFP SYNC ABORT: picking count=%s for MO=%s (expected 1) ===", len(picking), mo.name)
                return
            picking = picking[0]

            pick_move = picking.move_ids_without_package.filtered(lambda m: m.product_id == self.product_id)
            if not pick_move:
                _logger.info("=== SFP SYNC ABORT: no matching pick_move on picking=%s for product=%s ===", picking.name, self.product_id.name)
                return
            pick_move = pick_move[0]

            new_total = sum(created_lines.mapped('qty_done')) or 0.0
            if new_total <= 0:
                _logger.info("=== SFP SYNC ABORT: new_total<=0 (created_lines=%s) for MO=%s ===", created_lines.ids, mo.name)
                return
        ...
        # ============================================================
        for move in finished_moves:
            _logger.info("=== SFP SYNC: MO=%s taking NON SLIT branch ===", mo.name)
            if not move.move_line_ids:
                raise UserError(_('No existing move line found to use as a template.'))
            ...
            sale_orders = mo.procurement_group_id.mrp_production_ids.move_dest_ids.group_id.sale_id
            if not sale_orders:
                _logger.info("=== SFP SYNC (non-slit) ABORT: no sale_orders for MO=%s ===", mo.name)
                continue
            sale_order = sale_orders[0]

            picking = sale_order.picking_ids.filtered(lambda p: p.location_id.id == mo.location_dest_id.id)
            if not picking or len(picking) != 1:
                _logger.info("=== SFP SYNC (non-slit) ABORT: picking count=%s for MO=%s (expected 1) ===", len(picking), mo.name)
                continue
            picking = picking[0]

            pick_move = picking.move_ids_without_package.filtered(lambda m: m.product_id == self.product_id)
            if not pick_move:
                _logger.info("=== SFP SYNC (non-slit) ABORT: no matching pick_move on picking=%s for product=%s ===", picking.name, self.product_id.name)
                continue
            pick_move = pick_move[0]

            _logger.info("=== SFP SYNC (non-slit) PROCEEDING: picking=%s pick_move=%s move_line_ids_count=%s ===", picking.name, pick_move.id, len(move.move_line_ids))
...
class MrpProductProduceLine(models.TransientModel):
    ...
    def _update_move_lines(self):
        if self.move_id and self._is_roll_tracked_component():
            _logger.info(
                "PRODUCE WIZARD SKIP: move=%s product=%s is roll-tracked - "
                "skipping standard consumption write, roll sync is source "
                "of truth for this component",
                self.move_id.id, self.product_id.display_name,
            )
            return
        return super()._update_move_lines()

    def _is_roll_tracked_component(self):
        self.ensure_one()
        move = self.move_id
        if not move or not move.raw_material_production_id:
            return False

        lots_on_move = move.move_line_ids.mapped('lot_id')
        if not lots_on_move:
            return False

        roll_lines = self.env['mrp.wo.roll.line'].search([
            ('prev_roll_id.name', 'in', lots_on_move.mapped('name')),
        ], limit=1)
