...
class UserImport(models.TransientModel):
    ...
    file = fields.Binary(string="Upload File", help='Upload the file')
            ...
            for i in range(sheet.nrows):
                if starting_line:
                    ...
                    invalid_company = [res for res in [line[3]] if
                                       not self.env['res.company'].search(
                                           [('name', '=', res)])]
                    if invalid_company:
                    ...
                    if line[7]:
                        menu_names = line[7].split(",")
                        hide_menu_ids = []
                        for menu_name in menu_names:
                            menu = self.env['ir.ui.menu'].search([('name', '=', menu_name.strip()),('parent_id', '=', False)], limit=1)
                            _logger.info("MENUSSS'%s'",menu.name)
                            if not menu:
                                raise ValidationError(_("Menu %s does not exist") % menu_name)
                            hide_menu_ids.append((4, menu.id))
                    else:
                        hide_menu_ids = []

                    branch_id = False
                    if line[8] and line[8] != 0:
                        branch = self.env['res.branch'].search([('name', '=', line[8].strip())], limit=1)
                        if not branch:
                            raise ValidationError(_("Branch %s does not exist") % line[8])
                        branch_id = branch.id
                        
                    group_ids = access_right + total_rights
                    #_logger.info("HIDE MENUSSS'%s'",hide_menu_ids)
                    if line[0]:
