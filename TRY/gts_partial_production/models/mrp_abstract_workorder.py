...
class MrpAbstractWorkorderLine(models.AbstractModel):
    ...
    def _update_move_lines(self):
        ...
        self.ensure_one()
        if self.lot_id:
            move_lines = self.move_id.move_line_ids.filtered(lambda ml: ml.lot_id == self.lot_id and not ml.produce_line_ids)
        else:
            move_lines = self.move_id.move_line_ids.filtered(lambda ml: not ml.lot_id and not ml.produce_line_ids)
