...
class MRPProduction(models.Model):
    ...
    #     # IMPORTANT: x_order_master_yards write must trigger sync explicitly
    #     if 'x_order_master_yards' in vals:
    #         self._inverse_x_order_master_yards()

    #     # Your previous "new moves" logic (unchanged)
    #     if pre_move_ids_by_production:
    #         for production in self:
    #             pre_move_ids = pre_move_ids_by_production.get(production.id, set())
    #             new_moves = production.move_raw_ids.filtered(lambda m: m.id not in pre_move_ids)
    ...
    #             if new_moves and production.state != 'draft':
    #                 warehouse = production.picking_type_id.warehouse_id or getattr(production, 'warehouse_id', False)
    #                 pick_components = production.picking_ids.filtered(
    #                     lambda picking: picking.picking_type_id and (
    #                         (warehouse and picking.picking_type_id == warehouse.pbm_type_id)
    #                         or picking.picking_type_id.sequence_code == 'PC'
    #                     )
    #                 )
    #                 if pick_components:
    #                     # Use the first sequence-sorted workorder's workcenter location
    #                     first_wo = production.workorder_ids.sorted('sequence')[:1]
    #                     target_location = (
    #                         first_wo.workcenter_id.location_id
    #                         if first_wo and first_wo.workcenter_id.location_id
    #                         else pick_components[:1].location_dest_id  # fallback
    #                     )
    #                     _logger.info(
    #                         "[MO %s] new moves target_location=%s (from wo=%s)",
    #                         production.name,
    #                         target_location.complete_name if target_location else None,
    #                         first_wo.name if first_wo else None,
    #                     )
    #                     moves_to_reassign = new_moves.filtered(lambda m: m.state not in ('done', 'cancel'))
    #                 # if pick_components:
    #                 #     target_location = pick_components[:1].location_dest_id
    #                 #     moves_to_reassign = new_moves.filtered(lambda m: m.state not in ('done', 'cancel'))
    #                 #     for move in moves_to_reassign:
    #                 #         if move.location_dest_id != target_location:
    #                 #             move.write({'location_dest_id': target_location.id})
    #                 #         if move.picking_id and move.picking_id not in pick_components:
    #                 #             move.write({'picking_id': False})
    #                 #     moves_to_reassign.filtered(lambda m: not m.picking_id)._assign_picking()
    ...
    #     for production in self:
    #         if not production.picking_ids:
    #             continue

    #         # Your old "wo_name" logic
    #         vals = {}
    #         first_workorder = production.workorder_ids.sorted('sequence')[:1]
    #         if first_workorder:
    #             vals['wo_name'] = first_workorder.workcenter_id.name_get()[0][1]
    ...
    #         # (NEW) Ensure qty sync after confirm, because moves/WOs/pickings often exist only now
    #         if production.x_order_master_yards:
    #             production._inverse_x_order_master_yards()

    #         # Your old "location_dest_id" logic, but strengthened:
    #         # update picking header + stock moves + move lines, then reassign
    #         if first_workorder and first_workorder.workcenter_id.location_id:
    #             workcenter_location = first_workorder.workcenter_id.location_id
    #             warehouse = production.picking_type_id.warehouse_id or getattr(production, 'warehouse_id', False)
    #             pick_components = production.picking_ids.filtered(
    #                 lambda picking: picking.picking_type_id and (
    #                     (warehouse and picking.picking_type_id == warehouse.pbm_type_id)
    ...
    #             if pick_components:
    #                 # 1) Header (optional)
    #                 pick_components.write({'location_dest_id': workcenter_location.id})

    #                 # 2) Moves (this is what actually matters)
    #                 pc_moves = pick_components.mapped('move_ids_without_package').filtered(
    #                     lambda m: m.state not in ('done', 'cancel')
    #                 )
    #                 pc_moves.write({'location_dest_id': workcenter_location.id})

    #                 # 3) Move lines (if already created)
