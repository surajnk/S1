...
class RollLinesWiz(models.TransientModel):
    ...
    def _get_roll_base_and_consumed(self, workorder):
        """
        Returns (base_qty, saved_consumed, wizard_consumed) for self.prev_roll_id.

        base_qty        — quantity available at this WO from prev_roll_line_ids.
                          Falls back to roll.total_qty if not found.
        saved_consumed  — already consumed in saved workorder.roll_line_ids.
        wizard_consumed — consumed by OTHER lines in this wizard session (not self).
        """
        base_qty = 0.0
        if workorder:
...
class FinalRollLinesWiz(models.TransientModel):
    ...
    consumed_qty = fields.Float(string="Consumed Qty")
    remained_qty = fields.Float(string="Remained Qty", compute="_compute_remained_qty")
