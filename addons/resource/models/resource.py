...
class ResourceCalendar(models.Model):
        ...
        for attendance in self.env['resource.calendar.attendance'].search(domain):
            for resource in resources_list:
                ...
                for day in days:
                    ...
                    #_logger.info("Attendance Interval: Resource ID: %s, Start: %s, End: %s, Attendance: %s", resource.id, max(cache_dates[(tz, start_dt)], dt0), min(cache_dates[(tz, end_dt)], dt1), attendance)
    ...
    def _leave_intervals_batch(self, start_dt, end_dt, resources=None, domain=None, tz=None):
        ...
        for leave in self.env['resource.calendar.leaves'].search(domain):
            for resource in resources_list:
                ...
                #_logger.info("Leave Interval: Resource ID: %s, Start: %s, End: %s, Leave: %s", resource.id, max(start, dt0), min(end, dt1), leave)
    ...
    def _work_intervals_batch(self, start_dt, end_dt, resources=None, domain=None, tz=None):
        ...
        # _logger.info("Start datetime: %s, End datetime: %s", start_dt, end_dt)
        # _logger.info("Resources: %s", resources_list)

        attendance_intervals = self._attendance_intervals_batch(start_dt, end_dt, resources, tz=tz)
        #_logger.info("Attendance Intervals: %s", attendance_intervals)

        leave_intervals = self._leave_intervals_batch(start_dt, end_dt, resources, domain, tz=tz)
        #_logger.info("Leave Intervals: %s", leave_intervals)

        effective_intervals = {
            r.id: (attendance_intervals[r.id] - leave_intervals[r.id]) for r in resources_list
        }

        # for resource_id, intervals in effective_intervals.items():
        #     for interval in intervals:
                #_logger.info("Effective Work Interval for Resource %s: Start %s, End %s", resource_id, interval[0], interval[1])
    ...
    #         resources = self.env['resource.resource']
    #         resources_list = [resources]
    #         _logger.info("WITHIN IFFF RESOURCE LIST '%s'", resources_list)

    #     else:
    #         resources_list = list(resources)
    #         _logger.info("WITHIN ELSEEEE RESOURCE LIST '%s'", resources_list)
    ...
    def _get_days_data(self, intervals, day_total):
        ...
        for start, stop, meta in intervals:
            #_logger.info("Interval: Start %s, Stop %s, Duration %.2f hours",start, stop, (stop - start).total_seconds() / 3600)
            day_hours[start.date()] += (stop - start).total_seconds() / 3600
        #_logger.info("Accumulated Day Hours: %s", dict(day_hours))
        # compute number of days as quarters
        days = sum(
            float_utils.round(ROUNDING_FACTOR * day_hours[day] / day_total[day]) / ROUNDING_FACTOR if day_total[day] else 0
            for day in day_hours
        )
        #_logger.info("Computed Days: %.2f", days)
        return {
            'days': days,
    ...
    def get_work_duration_data(self, from_datetime, to_datetime, compute_leaves=True, domain=None):
        ...
        #_logger.info("From Datetime: %s, To Datetime: %s", from_datetime, to_datetime)
        ...
        if compute_leaves:
            ...
            #_logger.info("WITHIN IFFFF")
        else:
            intervals = self._attendance_intervals_batch(from_datetime, to_datetime)[False]
            #_logger.info("WITHIN ELSEEEEE")

        #_logger.info("Intervals: %s", intervals)

        result = self._get_days_data(intervals, day_total)
        #_logger.info("Work Duration Data: %s", result)
    ...
    def plan_hours(self, hours, day_dt, compute_leaves=False, domain=None, resource=None):
        ...
        if compute_leaves:
            ...
            _logger.info("PLANN HOURSS IFFF")
        else:
            get_intervals = self._attendance_intervals
            _logger.info("PLANN HOURSS ELSEEEE")

        _logger.info("PLANN HOURSS HOURSSSSS '%s'",hours)
        if hours >= 0:
            _logger.info("PLANN HOURSS HOURSSSSS IFFFFF")
            ...
            _logger.info("PLANN HOURSS HOURSSSSS ELSEEEE")
            hours = abs(hours)
            delta = timedelta(days=14)
            for n in range(100):
                dt = day_dt - delta * n
