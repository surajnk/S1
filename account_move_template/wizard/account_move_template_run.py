...
class AccountMoveTemplateRun(models.TransientModel):
    ...
    overwrite = fields.Text(
        help="""Valid dictionary to overwrite template lines:{'L1': {'partner_id': 1, 'amount': 100, 'name': 'some label'}, 'L2': {'partner_id': 2, 'amount': 200, 'name': 'some label 2'}, }
        """
    )
    ...
    total_debit = fields.Float(
        ...
        help="Sum of all debit lines in the wizard"
    )
    total_credit = fields.Float(
        string="Total Credit",
        compute="_compute_totals",
        help="Sum of all credit lines in the wizard"
    )

    @api.depends('line_ids.move_line_type', 'line_ids.amount')
    ...
    def _prepare_wizard_fixed_line(self, tmpl_line):
        vals = {
            "wizard_id": self.id,
            "sequence": tmpl_line.sequence,
            "name": tmpl_line.name,
            "amount": tmpl_line.value,
            "account_id": tmpl_line.account_id.id,
            "partner_id": tmpl_line.partner_id.id or False,
            "move_line_type": tmpl_line.move_line_type,
            "tax_line_id": tmpl_line.tax_line_id.id,
    ...
    def load_lines(self):
        ...
        for tmpl_line in tmpl_lines.filtered(lambda l: l.type == "input"):
            ...
            amtlro.create(vals)
        for tmpl_line in tmpl_lines.filtered(lambda l: l.type == "fixed"):
            vals = self._prepare_wizard_fixed_line(tmpl_line)
            amtlro.create(vals)
        self.write(
            {
    ...
    def generate_move(self):
        ...
        print('sequence2amount',sequence2amount)
