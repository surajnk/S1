...
class TestStockBarcodesPicking(TestStockBarcodes):
    ...
    def setUpClass(cls):
        ...
        cls.barcode_option_group_out.show_pending_moves = True
        cls.barcode_option_group_in.show_pending_moves = True
    ...
    def test_pending_moves_update_with_multiple_lots(self):
        picking = self.picking_out_01.copy()
        ...
        wiz_scan = self.ScanReadPicking.browse(action["res_id"]).with_context(
            force_create_move=True
        )
        wiz_scan.fill_pending_moves()
        pending_move = wiz_scan.pending_move_ids[0]
        self.assertEqual(pending_move.qty_done, 0.0)

        self.action_barcode_scanned(wiz_scan, self.product_tracking.barcode)
        self.action_barcode_scanned(wiz_scan, self.lot_1.name)
        pending_move.invalidate_cache()
        self.assertEqual(pending_move.qty_done, 1.0)

        self.action_barcode_scanned(wiz_scan, self.product_tracking.barcode)
        self.action_barcode_scanned(wiz_scan, lot_2.name)
        pending_move.invalidate_cache()
        self.assertEqual(pending_move.qty_done, 2.0)
