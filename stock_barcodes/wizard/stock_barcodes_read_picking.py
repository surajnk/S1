...
class WizStockBarcodesReadPicking(models.TransientModel):
    ...
    @api.depends("todo_line_ids", "todo_line_ids.qty_done", "todo_line_ids.state")
    ...
    def create(self, vals):
        # Ensure picking_mode is always set before super().create() so
        # _field_candidate_ids never resolves to "candidate_None_ids".
        if not vals.get("picking_mode"):
            vals["picking_mode"] = "picking"

        if not vals.get('option_group_id'):
            og_id = self._default_option_group_id()  # this will see context
            # if still nothing, try again with vals (so _resolve_picking_code can read them)
            if not og_id:
                code = self._resolve_picking_code(vals)
                ...
                if grp_code:
                    ...
                    og_id = grp.id if grp else False
            if og_id:
                vals['option_group_id'] = og_id
        wiz = super().create(vals)
        if not wiz.option_group_id:  # safety net
            wiz.option_group_id = self._default_option_group_id()
        return wiz


    # @api.model
    # def create(self, vals):
    #     # ensure group is there even if the action/JS didn’t pass defaults
    #     if not vals.get("option_group_id"):
    #         og_id = self._default_option_group_id()  # inherited from base
    #         if og_id:
    #             vals["option_group_id"] = og_id
    #     wiz = super().create(vals)
    #     # safety net if still empty
    #     if not wiz.option_group_id:
    #         og_id = wiz._default_option_group_id()
    #         if og_id:
    #             wiz.option_group_id = og_id
    #     if wiz.picking_id:
    ...
    def set_info_from_quants(self, quants):
        """Keep lot scan quantity aligned with transfer demand in picking mode."""
        res = super().set_info_from_quants(quants)
        # Keep guided_location_id in sync when fill_fields_from_lot
        # moves the stock to a different location than the move default
        if (
            self.location_id
            ...
            self.guided_location_id = self.location_id
        if (
            self.picking_id
            and self.product_id
            and self.product_qty
            and not self.is_manual_qty
            and self.option_group_id.code not in ["OUT", "INV"]
        ):
            pending_qty = sum(
                self.picking_id.move_lines.filtered(
                    lambda mv: mv.product_id == self.product_id
                    and mv.state in self._states_move_allowed()
                ).mapped(lambda mv: max(mv.product_uom_qty - mv.quantity_done, 0.0))
            )
            if pending_qty > 0:
                self.product_qty = min(self.product_qty, pending_qty)
        # Sync destination from picking in case it was updated by workcenter sync
        if self.picking_id and self.picking_id.location_dest_id:
    ...
    def determine_todo_action(self, forced_todo_line=False):
        ...
        self.guided_location_dest_id = (self.picking_id.location_dest_id if self.picking_id else move_line.location_dest_id)
        ...
        if self.option_group_id.get_option_value("location_dest_id", "filled_default"):
            self.location_dest_id = (self.picking_id.location_dest_id if self.picking_id else move_line.location_dest_id)
    ...
    def action_done(self):
        # Flush pending writes so with_context() instance reads fresh values
        _logger.info("NEWCODE_V2_action_done_loaded")
        self.env["wiz.stock.barcodes.read.picking"].flush(
            ["product_qty", "lot_id", "location_id", "location_dest_id",
             "product_id", "guided_product_id", "guided_location_id",
             "guided_location_dest_id", "guided_lot_id"]
        )
        res = super(
        ...
        _logger.info(
            "action_done: super()=%s, picking=%s, product=%s, lot=%s, qty=%s",
            res, self.picking_id.name, self.product_id.name,
            self.lot_id.name, self.product_qty,
        )
        if res:
            move_dic = self._process_stock_move_line()
            _logger.info("action_done: move_dic=%s", move_dic)
            if move_dic:
    ...
    def action_confirm(self):
        ...
        preserve_fields = {
            ...
            "guided_product_id": self.guided_product_id.id,
            "guided_location_id": self.guided_location_id.id,
            "guided_location_dest_id": self.guided_location_dest_id.id,
            "guided_lot_id": self.guided_lot_id.id,
        }
        for field_name, field_value in preserve_fields.items():
            if field_value and not vals.get(field_name):
                vals[field_name] = field_value

        record.write(vals)
        self = record

        # Restore candidate pickings if lost from cache
        if self.picking_id and not self.candidate_picking_ids:
            self._set_candidate_pickings(self.picking_id)

        self.env["wiz.stock.barcodes.read.picking"].flush(
            ["product_qty", "lot_id", "location_id", "location_dest_id",
             "product_id", "guided_product_id", "guided_location_id",
             "guided_location_dest_id", "guided_lot_id", "candidate_picking_ids"]
        )

        res = self.action_done()

        if self.option_group_id.show_pending_moves and self.todo_line_ids:
            self.env["stock.move.line"].flush(["qty_done", "barcode_scan_state"])
            self.todo_line_ids.invalidate_cache(
                ["qty_done", "state", "pending_product_qty"], self.todo_line_ids.ids
            )
            self.todo_line_ids._compute_qty_done()
            self.todo_line_ids._compute_state()
            self.todo_line_ids._compute_pending_product_qty()
    ...
    #     # Preserve critical wizard values that can be dropped from form cache
    #     # when fields are invisible on the active barcode step.
    #     preserve_fields = {
    #         "picking_id": self.picking_id.id,
    #         "picking_type_code": self.picking_type_code,
    #         "picking_mode": self.picking_mode,
    ...
    def _prepare_move_line_values(self, candidate_move, available_qty):
        ...
        vals = {
            ...
            "product_uom_qty": 0.0 if self.picking_type_code == "internal" else available_qty,
            ...
            "location_dest_id": self.location_dest_id.id,
            "lot_id": self.lot_id.id,
            "lot_name": self.lot_id.name if self.picking_type_code == "incoming" else False,
            "barcode_scan_state": "done_forced"
            if self.env.context.get("done_forced")
            else "done",
            "package_id": self.package_id.id,
            "result_package_id": self.result_package_id.id,
        }
    ...
    def _process_stock_move_line(self):  # noqa: C901
        """
        Allow multi-lot scanning to fulfill one move:
        if no candidate line matches the scanned lot and there is remaining demand,
        create a new stock move line for that lot.
        """
        _logger.info(
            "_process_stock_move_line: picking=%s, product=%s, lot=%s, "
            "location=%s, location_dest=%s, qty=%s, picking_type_code=%s",
            self.picking_id.name, self.product_id.name, self.lot_id.name,
            self.location_id.name, self.location_dest_id.name,
            self.product_qty, self.picking_type_code,
        )
        ...
        candidate_result = getattr(self, "_search_candidate_%s" % self.picking_mode)(moves_todo)
        ...
        _logger.info(
            "candidate_lines: %s",
            [(l.id, l.lot_id.name, l.location_id.name, 
              l.location_dest_id.name, l.barcode_scan_state,
              l.product_uom_qty, l.qty_done) 
             for l in candidate_lines]
        )
        _logger.info(
            "all picking move_line_ids: %s",
            [(l.id, l.lot_id.name if l.lot_id else False, 
              l.location_id.name, l.location_dest_id.name,
              l.barcode_scan_state, l.product_uom_qty, l.qty_done)
             for l in self.picking_id.move_line_ids]
        )
        _logger.info(
            "wizard: location=%s location_dest=%s",
            self.location_id.name, self.location_dest_id.name
        )
        allowed_states = ["pending"]
        if self.env.context.get("done_forced"):
            allowed_states.extend(["done", "done_forced"])
        if self.picking_type_code == "internal" and "done" not in allowed_states:
            allowed_states.append("done")

        lines = candidate_lines.filtered(
            lambda l: l.lot_id == self.lot_id and l.barcode_scan_state in allowed_states
        )

        # Serial: avoid duplicate serial in the same picking
        ...
        # Incoming: allow filling missing lot with the scanned lot
        if (
            not lines
            ...
            if lines:
                sml_vals.update({"lot_id": self.lot_id.id, "lot_name": self.lot_id.name})

        # Apply extra candidate domain (packages, locations, etc.)
        candidate_domain = self._get_candidate_line_domain()
        if candidate_domain:
            lines = lines.filtered_domain(candidate_domain)

        # Consider lines created without reserved qty for this lot
        if not lines:
            lines = candidate_lines.filtered(
                lambda ln: (
        ...
        if (
            ...
            and float_compare(
                available_qty, max_quantity, precision_rounding=self.product_id.uom_id.rounding
            )
            > 0
        ):
        ...
        # Allocate into existing matching lines
        for line in lines:
        ...
        # No matching candidate line for this lot but demand exists -> create or update SML
        if (
            not lines
            and self.product_id.tracking == "lot"
            and self.lot_id
            and float_compare(
                available_qty, 0.0, precision_rounding=self.product_id.uom_id.rounding
            ) > 0
            and float_compare(
                max_quantity, 0.0, precision_rounding=self.product_id.uom_id.rounding
            ) > 0
        ):
            # Search ALL move lines in the picking for this lot (not just candidate_lines)
            # to catch Odoo-reserved lines that may be filtered out by location/state
            existing = self.picking_id.move_line_ids.filtered(
                lambda sml: sml.lot_id == self.lot_id
                and sml.product_id == self.product_id
                and sml.state not in ("done", "cancel")
            )
            if existing:
                # Update the existing reserved line instead of creating a duplicate
                sml = existing[0]
                new_qty = sml.qty_done + available_qty
                self._update_stock_move_line(sml, {"qty_done": new_qty})
                if new_qty >= sml.product_uom_qty:
                    sml.barcode_scan_state = "done"
                move_lines_dic[sml.id] = available_qty
                available_qty = 0.0
            else:
                new_smls = self.create_new_stock_move_line(moves_todo, available_qty)
                for sml in new_smls:
                    if not sml.move_id:
                        self.create_new_stock_move(sml)
                    # lot_name=False prevents Odoo from trying to create a new lot record
                    self._update_stock_move_line(sml, {
                        "lot_id": self.lot_id.id,
                        "lot_name": False,
                    })
                ...
                if self.option_group_id.barcode_guided_mode == "guided":
                    self.todo_line_id.line_ids = [(4, sml.id) for sml in new_smls]
                elif self.option_group_id.show_pending_moves:
                    self.fill_todo_records()

                available_qty = 0.0

        # If residual qty still remains, use standard extra-line path
        if float_compare(
            available_qty, 0, precision_rounding=self.product_id.uom_id.rounding
        ) > 0:
            stock_move_lines = self.create_new_stock_move_line(moves_todo, available_qty)
    ...
    def update_fields_after_process_stock(self, moves):
        ...
        _logger.info(
            "update_fields_after_process_stock: show_pending_moves=%s, "
            "todo_line_ids count=%s, SML qty_done=%s",
            self.option_group_id.show_pending_moves,
            len(self.todo_line_ids),
            self.picking_id.move_line_ids.mapped("qty_done"),
        )
        if self.option_group_id.show_pending_moves:
            # Flush pending ORM writes to DB so recomputes read fresh values
            self.env["stock.move.line"].flush(["qty_done", "barcode_scan_state"])
            self.todo_line_ids.invalidate_cache(
                ["qty_done", "state", "pending_product_qty"]
            )
            self.todo_line_ids._compute_qty_done()
            self.todo_line_ids._compute_state()
            self.todo_line_ids._compute_pending_product_qty()
            self._compute_pending_move_ids()
    # def update_fields_after_process_stock(self, moves):
    #     self.picking_product_qty = sum(moves.mapped("quantity_done"))
    #     if self.option_group_id.show_pending_moves:
    #         self.todo_line_ids.invalidate_cache(
    ...
    def check_done_conditions(self):
        ...
        _logger.info("PICKING check_done_conditions: super()=%s", res)
        if (
            ...
            _logger.info(
                "PICKING check_done_conditions: FAILED qty check "
                "product_qty=%s qty_available=%s picking_type_code=%s allow_negative=%s",
                self.product_qty, self.qty_available,
                self.picking_type_code,
                self.option_group_id.allow_negative_quant,
            )
            self._set_messagge_info(
        ...
        if not self.picking_id:
            if not self._search_candidate_picking():
                _logger.info("PICKING check_done_conditions: FAILED no candidate picking")
                self._set_messagge_info(
                    "info", _("Click on picking pushpin to lock it")
                )
                return False
        if self.picking_id and self.picking_id != self._candidate_picking_selected():
            _logger.info("PICKING check_done_conditions: FAILED picking not selected")
            self._set_messagge_info("info", _("Click on picking pushpin to lock it"))
            return False
        _logger.info("PICKING check_done_conditions: PASSED")
        return res
