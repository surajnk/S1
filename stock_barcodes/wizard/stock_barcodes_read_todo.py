...
class WizStockBarcodesReadTodo(models.TransientModel):
    ...
    pending_product_qty = fields.Float(
        "Pending",
        digits="Product Unit of Measure",
        compute="_compute_pending_product_qty",
        store=True,
    )
    ...
    def _prepare_fill_record_values(self, wiz_barcode, line, position):
        ...
        if wiz_barcode.option_group_id.source_pending_moves == "move_line_ids":
            ...
            # Use picking's location_dest_id if available (may be synced to workcenter)
            move_location_dest = (line.move_line_ids[:1] or line).location_dest_id
            picking_location_dest = line.picking_id.location_dest_id
            resolved_dest = picking_location_dest or move_location_dest
            vals.update(
                {
                    "location_id": (line.move_line_ids[:1] or line).location_id.id,
                    "location_dest_id": resolved_dest.id,
    ...
    def _get_relevant_move_lines(self):
        self.ensure_one()
        if self.stock_move_ids:
            move_lines = self.stock_move_ids.mapped("move_line_ids")
        else:
            move_lines = self.line_ids
        if self.wiz_barcode_id.option_group_id.source_pending_moves == "move_line_ids":
            move_lines = move_lines.filtered(
                lambda ln: ln.product_id == self.product_id
                and ln.location_id == self.location_id
                and ln.location_dest_id == self.location_dest_id
                and ln.lot_id == self.lot_id
                and ln.package_id == self.package_id
                and ln.result_package_id == self.result_package_id
            )
        else:
            move_lines = move_lines.filtered(lambda ln: ln.product_id == self.product_id)
        return move_lines

    @api.depends(
        "line_ids.qty_done",
        "line_ids.barcode_scan_state",
        "stock_move_ids.move_line_ids.qty_done",
        "stock_move_ids.move_line_ids.barcode_scan_state",
    )
    def _compute_qty_done(self):
        for rec in self:
            move_lines = rec._get_relevant_move_lines()
            if move_lines:
                move_lines.invalidate_cache(["qty_done"], move_lines.ids)
            rec.qty_done = sum(move_lines.mapped("qty_done"))
    ...
    # def _compute_qty_done(self):
    #     for rec in self:
    #         rec.qty_done = sum(rec._get_relevant_move_lines().mapped("qty_done"))

    @api.depends("product_uom_qty", "qty_done")
    def _compute_pending_product_qty(self):
        for rec in self:
            rec.pending_product_qty = max(rec.product_uom_qty - rec.qty_done, 0.0)
    ...
    @api.depends(
        ...
        "stock_move_ids.move_line_ids.barcode_scan_state",
        "stock_move_ids.move_line_ids.qty_done",
        "stock_move_ids.move_line_ids.product_uom_qty",
        "qty_done",
        "product_uom_qty",
    )
    def _compute_state(self):
        for rec in self:
            lines = rec._get_relevant_move_lines()
            if rec.qty_done >= rec.product_uom_qty or (
                rec.wiz_barcode_id.option_group_id.source_pending_moves
                == "move_line_ids"
                and lines
                and not any(ln.barcode_scan_state == "pending" for ln in lines)
            ):
                rec.state = "done"
            else:
                rec.state = "pending"
    ...
    def fill_from_pending_line(self):
        ...
        # Override destination with picking's synced location (e.g. workcenter)
        if (
            self.wiz_barcode_id.picking_id
            and self.wiz_barcode_id.picking_id.location_dest_id
        ):
            self.wiz_barcode_id.location_dest_id = (
                self.wiz_barcode_id.picking_id.location_dest_id
            )
