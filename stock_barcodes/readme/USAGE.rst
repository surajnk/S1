...
Handling demand with multiple lots
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

When the quantity available on the first scanned lot is lower than the
required demand, keep the product selected and repeat the scan workflow with
additional lots until the needed quantity is reached:

#. Scan the **product**.
#. Scan **Lot A** even if it only partially covers the demand; the interface
   records the quantity done for that lot and updates the pending quantity.
#. Click the **Clean lot** (trash) action to clear the lot field for the same
   product.
#. Scan **Lot B** (and further lots as needed). Each lot creates or updates its
   own move line so the total ``qty_done`` accumulates toward the demand.

The pending list refreshes after every lot scan, so once the accumulated
``qty_done`` matches the demand, the move disappears from the "Pending moves"
