...
class MrpWorkorder(models.Model):
    ...
    def _get_chemical_cost(self, consumed_qty):
        ...
        if not wo_operation:
            _logger.warning(
                "WO %s: no operation_id set — skipping chemical cost.",
                self.name,
            )
            return 0.0
        ...
        for bom_line in bom.bom_line_ids:
            ...
            # ── Condition 1: UoM must be lbs ─────────────────────────────
            uom_name = (bom_line.product_uom_id.name or '').lower().strip()
            if uom_name not in ('lb', 'lbs'):
                continue

            # ── Condition 2: This WO's operation must be in workcenter_capacities
    ...
    def _create_pick_and_moves_for_output_rolls(
        ...
        # Skip action_confirm entirely — set states manually
        # action_confirm triggers FIFO reservation internally
        picking.write({'state': 'assigned'})

        for mv in moves_created:
            lot = move_lot_map.get(mv.id)
            lot_record = self.env['stock.production.lot'].browse(lot) if lot else None
        ...
        _logger.info("Outbound picking %s state=%s", picking.name, picking.state)
        return picking
    ...
    def get_previous_rolls(self):
        ...
        def _process_one_wo(wo):
            ...
            pickings = wo.production_id.picking_ids.filtered(
                lambda p: p.state == 'done' and not p.is_reverse_transfer)
