...
class AccountPaymentRegister(models.TransientModel):
    ...
    def _reconcile_open_invoices(
        ...
        if inv.move_type == "out_invoice":
            debit_move = line
            credit_move = payment_line
        else:
            debit_move = payment_line
            credit_move = line

        acc_part_recnc_obj.create({
    ...
    def make_payments(self):
        ...
        if not self.group_payment:
            ...
            for partner_id, partner_data in group_data.items():
                for invoice_id, invoice_vals in partner_data["inv_val"].items():
                    # Set context with only this invoice
                    context.update({"active_ids": [invoice_id]})

                    # Build a custom group_data just for this invoice
                    single_invoice_group = {
                        "partner_id": partner_id,
                        "partner_type": partner_data["partner_type"],
                        "total": invoice_vals["amount"],
                        "check_amount_in_words": partner_data["check_amount_in_words"],
                        "memo": partner_data["memo"],
                        "inv_val": {invoice_id: invoice_vals},
                    }
                    ...
                    payment = (
                        ...
                        .create(self.get_payment_values(group_data=single_invoice_group))
                    )
                    payment.action_post()
                    payment_ids.append(payment.id)

                    # Get lines for reconciliation
                    payment_lines = payment.line_ids.filtered_domain([
                        ("account_internal_type", "in", ("receivable", "payable")),
                        ("reconciled", "=", False),
                    ])
                    invoice = self.env["account.move"].browse(invoice_id)
                    invoice_lines = invoice.line_ids.filtered_domain([
                        ("account_internal_type", "in", ("receivable", "payable")),
                        ("reconciled", "=", False),
                    ])
                    ...
                    for account in payment_lines.account_id:
                        lines_to_reconcile = (payment_lines + invoice_lines).filtered_domain([
                            ("account_id", "=", account.id),
                            ("reconciled", "=", False),
                        ])
                        lines_to_reconcile.reconcile()

                    # Let Odoo compute correct payment state
                    invoice._compute_amount()
                    invoice._compute_payments_widget_reconciled_info()
        else:

            for group in list(group_data):
                if context.get("active_ids") and group_data[group].get("inv_val"):
                ...
                if any(
                        group_data[group]["inv_val"][inv.id]["payment_difference_handling"] == "open"
                        for inv in invoices
                ):
                    for inv in invoices:
                        if group_data[group]["inv_val"][inv.id]["payment_difference_handling"] == "open":
                            payment_state = "partial"
                        else:
                            payment_state = "paid"

                        for line in inv.line_ids.filtered_domain(domain):
                            print('line', line)
                            if line.amount_residual != 0  and payment_state == "paid":
                                ...
                                matched_payment_line = payment_lines.filtered(lambda pl: pl.account_id == line.account_id)
                                if matched_payment_line:
                                    self._reconcile_open_invoices(
                                        line,
                                        matched_payment_line[0],
                                        inv,
                                        line_amount,
                                        line_amount,
                                        True,
                                    ...
                                    continue
                            if line.amount_residual != 0 and payment_state == "partial":
                        ...
                        inv.update({
                            ...
                            "amount_residual": group_data[group]["inv_val"][inv.id]["payment_difference"],
                            "amount_residual_signed": group_data[group]["inv_val"][inv.id]["payment_difference"],
                        })
