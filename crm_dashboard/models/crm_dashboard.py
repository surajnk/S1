...
class CRMLead(models.Model):
    ...
    def lead_details_user(self):
        ...
        if revenue_value == 0:
            ratio_value = 0
        if revenue_value > 0:
            self._cr.execute('''select case when coalesce(a.count_one, 0) + coalesce(b.count_two, 0) = 0 then 0 else (
            CAST(coalesce(a.count_one, 0) as float) / CAST(coalesce(b.count_two, 0) as float))end as final_count
