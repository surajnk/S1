...
class AccountReconciliation(models.AbstractModel):
    ...
    def _prepare_move_lines(self, move_lines, target_currency=False, target_date=False, recs_count=0):
        ...
        for line in move_lines:
            ...
            # Apply early payment discount for receivable/payable lines only
            if (
                invoice
                ...
                discount_info = invoice.invoice_payment_term_id._check_payment_term_discount(
                    invoice=invoice, payment_date=payment_date
                )
                discount_amt = discount_info[0]
                if discount_amt > 0:
                    # Subtract discount while preserving correct sign
                    sign = -1 if amount < 0 else 1
                    amount -= sign * discount_amt
            ...
            if discount_amt > 0 and line.account_id.internal_type in ['payable', 'receivable']:
                sign = -1 if total_amount < 0 else 1
                total_amount -= sign * discount_amt
