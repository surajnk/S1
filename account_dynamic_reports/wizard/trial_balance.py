...
class InsTrialBalance(models.TransientModel):
    ...
    def process_filters(self, data):
        ...
        filters['branch'] = data.get('branch')
    ...
    def process_data(self, data):
        if data:
            ...
            move_lines = {
                x.code: {'name': x.name, 'code': x.code, 'group': x.group_id.id, 'group_name': x.group_id.name,
                         'id': x.id,
            ...
            for account in account_ids:
                ...
                WHERE_INIT = WHERE + " AND l.date >= '%s'" % data.get('date_from') + " AND l.date <= '%s'" % data.get(
                    'date_to')
                ...
                if self.branch_ids:
                    if len(self.branch_ids.ids) == 1:
                        WHERE_CURRENT += " AND res_branch.id = {}".format(tuple(self.branch_ids.ids)[0])
                    else:
                        WHERE_CURRENT += " AND res_branch.id in {}".format(tuple(self.branch_ids.ids))
                ...
                sql = ('''
                    ...
                    LEFT JOIN account_analytic_account anl ON (l.analytic_account_id=anl.id)
                    LEFT JOIN res_currency c ON (l.currency_id=c.id)
                    LEFT JOIN res_branch  ON (l.branch_id=res_branch.id)
                    LEFT JOIN res_partner p ON (l.partner_id=p.id)
                    JOIN account_journal j ON (l.journal_id=j.id)
                    WHERE %s
                ''') % WHERE_CURRENT
            ...
            for move in move_lines:
                if move_lines_ids.get(move_lines[move]['group_name']):
                    values = {
                        'name': move_lines[move]['name'],
                        'code': move_lines[move]['code'],
                        'group': move_lines[move]['group'],
                        'group_name': move_lines[move]['group_name'],
                        'id': move_lines[move]['id'],
                        'initial_debit': move_lines[move]['initial_debit'],
                        'initial_credit': move_lines[move]['initial_credit'],
                        'initial_balance': move_lines[move]['initial_balance'],
                        'debit': move_lines[move]['debit'],
                        'credit': move_lines[move]['credit'],
                        'balance': move_lines[move]['balance'],
                        'ending_credit': move_lines[move]['ending_credit'],
                        'ending_debit': move_lines[move]['ending_debit'],
                        'ending_balance': move_lines[move]['ending_balance'],
                        'company_currency_id': move_lines[move]['company_currency_id'],
                    }
                    move_lines_ids.get(move_lines[move]['group_name']).append(values)
                else:
                    values = [{
                        'name': move_lines[move]['name'],
                        'code': move_lines[move]['code'],
                    ...
                    move_lines_ids[move_lines[move]['group_name']] = values
            return [move_lines_ids, retained, subtotal]
    ...
    def get_filters(self, default_filters={}):
        ...
        branch = self.branch_ids if self.branch_ids else self.env['res.branch'].search(company_domain)
        ...
        for a in accounts:
            code = ''
            if a.branch_id:
                code = a.branch_id.name
            accounts_lst.append((a.id, a.name + ' ' + code))
        ...
        filter_dict = {
            ...
            'branch': [(a.id, a.name) for a in branch],
            'accounts_list': accounts_lst,
    ...
    def action_xlsx(self):
        ...
        if account_lines:
            if not filter.get('show_hierarchy'):
                for line in account_lines.get(False, []):
                    row_pos += 1
                    sheet.write(row_pos, 0,
                                line.get('code') + ' ' + line.get('name'),
                                line_header_light_left)
                    sheet.write(row_pos, 1, float(line.get('initial_debit')),
                                line_header_light)
                    sheet.write(row_pos, 2, float(line.get('initial_credit')),
                                line_header_light)
                    sheet.write(row_pos, 3, float(line.get('initial_balance')),
                                line_header_highlight)
                    sheet.write(row_pos, 4, float(line.get('debit')),
                                line_header_light)
                    sheet.write(row_pos, 5, float(line.get('credit')),
                                line_header_light)
                    sheet.write(row_pos, 6, float(line.get('balance')),
                                line_header_highlight)
                    sheet.write(row_pos, 7, float(line.get('ending_debit')),
                                line_header_light)
                    sheet.write(row_pos, 8, float(line.get('ending_credit')),
                                line_header_light)
        ...
        return {
            ...
            'url': '/web/content/common.xlsx.out/%s/filedata/%s?download=true' % (
                report_id.id, report_id.filename
            ),
            'target': 'new',
        }
        output.close()
