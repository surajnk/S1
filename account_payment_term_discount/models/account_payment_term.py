...
class AccountPaymentTerm(models.Model):
    ...
    def write(self, vals):
        res = super(AccountPaymentTerm, self).write(vals)
        if vals.get('is_discount') == False:
            for ln in self.line_ids:
                if ln.is_allow_give_discount:
                    ln.is_allow_give_discount = False
        return res

    @api.onchange('is_discount')
    def OnchangeisDiscount(self):
        if self.is_discount == False:
            #_logger.info("Boolean'%s'",rec.x_unit_req)
            for ln in self.line_ids:
                ln.is_allow_give_discount = False
                ln.discount = 0.000
                ln.discount_days = 0
                ln.discount_income_account_id = 0
                ln.discount_expense_account_id = 0
...
class AccountPaymentTermLine(models.Model):
    ...
    is_allow_give_discount = fields.Boolean('Discount Allowed after Due Date?')

    # @api.onchange('is_discount')
    # def OnchangeisDiscount(self):
    #     if self.is_discount == False:
    #         #_logger.info("Boolean'%s'",rec.x_unit_req)
    #         self.is_allow_give_discount = False
